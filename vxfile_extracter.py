...
def run_binwalk_extract(file_path):
    ...
    try:
        ...
        endian = "big" if "big endian" in output.lower() else "little" if "little endian" in output.lower() else "big"
...
def main(file_path):
    ...
    output, output_dir, endian = run_binwalk_extract(file_path)
    ...
    while True:
        ...
        file_info = extract_file_info(uboot_img_file_path, start_marker, endian)
        if file_info:
            break
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("用法：python3 vxfile_extracter.py <bin 文件路径>")
        sys.exit(1)
    ascii_art = """
    ██╗   ██╗██╗  ██╗███████╗██╗██╗     ███████╗        ███████╗██╗  ██╗████████╗██████╗  █████╗  ██████╗████████╗ ██████╗ ██████╗ 
    ██║   ██║╚██╗██╔╝██╔════╝██║██║     ██╔════╝        ██╔════╝╚██╗██╔╝╚══██╔══╝██╔══██╗██╔══██╗██╔════╝╚══██╔══╝██╔═══██╗██╔══██╗
